...
//TODO list
// run test without click the checkbox !!!!
// turn off VBAT and USB_ENABLE after each time start test for both PCB and PRODUCT test !!!!!!!!!!!!!!!!!!!!!!!!!!
//1. pairingclear
//2. read config file
//3. change all test_mode , debug_mode, fft_write to 0. 
//4. pairingInfoWrite 
//5. run spi lock if everything is ok
//6. test verifybluetoothaddr
//7. ask swetha about if using beta 1.8, usb-enable if used or not
//8. according jefferson, use latest email for serial number. 
//9. fix pass/failure color, error for 'no avail serial' and 'unable to write serial'.
//10. update bluetooth address after 'restore'. 
//11. copy latest bin.xpv xuv 
//12. fix log10 displaying problem. 
//13. test factory enable
//14. test turn off PC, bluefish still works or not
//15. read serial # as soon as power is turned on.
//16. split configuration for PCB speaker, PCB microphone, PRODUCT speaker, PRODUCT microphone.
//17. clear status before each test
//18. fix logs contents
...
1. Read Serial number prefix (10 char) from config file
2. Serial number prefix + date information (year : 2 char + month : 1 char) + 6 hex bluetooth address = serial number

*/
#define PRODUCT_SERIAL_NUM_PREFIX_STR									"PRODUCT_SERIAL_NUM_PREFIX"
#define PRODUCT_FACTORY_INDICATOR_STR									"PRODUCT_FACTORY_INDICATOR"
// PSKEY_USR40 is unit16 array, SN prefix will be stored in both uint16[] and char[], snPredixLen is the length of snPrefix (uint16)  
...
/*Seavia 
  * convert current year / month to char array. Ex. 2015 Oct = > 15A
  *
  */
void getDateMonth(char dst[4]) {
	...
	dst[2] = "012345678ABCDEF"[(now->tm_mon)+1];
	dst[3] = '\0';
}
...
#define NUM_OF_CONFIGURATIONS 53
#else
...
#if 1															 /* Seavia 20160512 add sn prefix*/
...
#if 1 /* Seavia 20150810 , fix unreferenced local variable warning*/
	int resultlow, result_freq_hopping;
#else
			...
			do 
			{
				...
				if (iSuccess == TE_OK)
				{
					...
					*/
					...
					convert2U16Arr(snPrefix_u16, snCandidate, snPrefixLen);
...
int ReadModelFile(char *file_name)
{
	...
	{	
		...
		{	
			...
			//modelsStrings[i] = new char[15];
			//memset(modelsStrings[i],0,15);
...
int ResetConfigFile(char *modelName, char *fileName)
{
	...
	{
		strcpy(fileName, configFileNames[1][1]);
		WriteMainLogFile("ResetConfigFile :: MVR02 matched, read config : %s", configFileNames[1][1]);
	}
	else if (strstr(modelName, "MVS02"))
	{
		strcpy(fileName, configFileNames[1][2]);
		WriteMainLogFile("ResetConfigFile :: MVS02 matched, read config : %s", configFileNames[1][2]);
	}
	else if (strstr(modelName, "MNR01")) //reserve for notifier
	{
		strcpy(fileName, configFileNames[1][0]);
		WriteMainLogFile("ResetConfigFile :: MNR01 matched, read config : %s", configFileNames[1][0]);
	}
	else if (strstr(modelName, "GVS02GCM"))
	{
		strcpy(fileName, configFileNames[1][3]);
		WriteMainLogFile("ResetConfigFile :: GVS02GCM matched, read config : %s", configFileNames[1][3]);
	}
	else if (strstr(modelName, "GVS02GCG"))
	{
		strcpy(fileName, configFileNames[1][4]);
		WriteMainLogFile("ResetConfigFile :: GVS02GCG matched, read config : %s", configFileNames[1][4]);
	}
	else
	{
...
int ReadConfigFile(char *file_name)
{
...
#if 1 /* Seavia 20150612 serial number prefix string*/
	// John: Globaling this, 
	...
	{	
		...
		{	
			...
			config_name_cstr = config_name.c_str();
				...
				{	
					if (!ConfigFile.fail())
					{	WriteLogFile("ReadConfigFile:config.txt is corrupted."); 
						WriteMainLogFile("ReadConfigFile:config.txt is corrupted."); 
						return /*0*/-1;
					}
				}
...
#if 1 // John 07302015: Getting Serial_Num_Prefix
	...
	{
		WriteLogFile("ReadConfigFile:config.txt not been able to be opened."); 
		WriteMainLogFile("ReadConfigFile:config.txt not been able to be opened."); 
		return 0;
	}
	ConfigFile.close();
	return 1;
...
#if 0 /*John 08132015: write barcode info to file*/

#define BARCODE_SERIAL_NUMBER_FILENAME "..\\BarcodeInfo\\BarcodeSerialNumber.txt"
#define BARCODE_BLUEADDR_FILENAME "..\\BarcodeInfo\\BarcodeBTAddr.txt"
int WriteBarcodeFile()
{
	ofstream AssignedBarcodeSNFile (BARCODE_SERIAL_NUMBER_FILENAME);
	ofstream AssignedBarcodeBTFile (BARCODE_BLUEADDR_FILENAME);
	// John is lazy
	if (AssignedBarcodeSNFile.fail() || AssignedBarcodeBTFile.fail())
	{	
		WriteLogFile("WriteAssignedSNFile:Can not create %s.", AssignedBarcodeSNFile); 
		WriteLogFile("AssignedBarcodeBTFile:Can not create %s.", AssignedBarcodeBTFile); 
		return 0;
	}
...
{	CString	strMessage, currentBluetoothAddrStr, logfileName, resultstring;
	...
	switch(pcb_tests_overall_result)
	{
		case SUCCESS:
			...
			label_text_color = GREEN;
			label_text_id = IDC_STATIC_PCB_TEST_RESULT;
			resultstring = "PCB TEST RESULT: PASS";
		...
		case NOT_STARTED:
			...
			label_text_id = IDC_STATIC_PCB_TEST_RESULT;
			resultstring = "PCB TEST RESULT: Not started";
			GetDlgItem(IDC_STATIC_PCB_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		case WRITE_PAIRING_INFO_FAILED:
			...
			label_text_color = RED;
			label_text_id = IDC_STATIC_PCB_TEST_RESULT;
			resultstring = "PCB TEST RESULT: FAILURE Write PairingInfo Failed";
			GetDlgItem(IDC_STATIC_PCB_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		case NO_AVAIL_SERIAL_NUMBER:
			...
			label_text_color = RED;
			label_text_id = IDC_STATIC_PCB_TEST_RESULT;
			resultstring = "PCB TEST RESULT: FAILURE No Avail SN";
			GetDlgItem(IDC_STATIC_PCB_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		case WRITE_SERIAL_NUMBER_FAILED:
			...
			label_text_color = RED;
			label_text_id = IDC_STATIC_PCB_TEST_RESULT;
			resultstring = "PCB TEST RESULT: FAILURE Write SN Failed";
			GetDlgItem(IDC_STATIC_PCB_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		case NOT_COMPLETE:
			...
			label_text_color = RED;
			label_text_id = IDC_STATIC_PCB_TEST_RESULT;
			resultstring = "PCB TEST RESULT: Not Complete";
			GetDlgItem(IDC_STATIC_PCB_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		default:
			...
			label_text_color = RED;
			label_text_id = IDC_STATIC_PCB_TEST_RESULT;
			resultstring = "PCB TEST RESULT: FAILURE";
			GetDlgItem(IDC_STATIC_PCB_TEST_RESULT)->SetWindowText(resultstring); 
	}
#ifdef update_color
...
//#endif
	...
	switch(product_tests_overall_result)
	{
		case SUCCESS:
			...
			label_text_color = GREEN;
			label_text_id = IDC_STATIC_PRODUCT_TEST_RESULT;
			resultstring = "PRODUCT TEST RESULT: PASS";
		...
		case NOT_STARTED:
			...
			label_text_color = BLACK;
			label_text_id = IDC_STATIC_PRODUCT_TEST_RESULT;
			resultstring = "PRODUCT TEST RESULT: Not started";
			GetDlgItem(IDC_STATIC_PRODUCT_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		case WRITE_BT_ADDR_OR_LICENSE_KEY_FAILED:
			...
			label_text_color = RED;
			label_text_id = IDC_STATIC_PRODUCT_TEST_RESULT;
			resultstring = "PRODUCT TEST RESULT: FAILURE Write Bt Addr or License Failed";
			GetDlgItem(IDC_STATIC_PRODUCT_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		case NO_AVAIL_BT_ADDR:
			label_text_color = RED;
			label_text_id = IDC_STATIC_PRODUCT_TEST_RESULT;
			resultstring = "PRODUCT TEST RESULT: FAILURE No Avail BT Addr";
			GetDlgItem(IDC_STATIC_PRODUCT_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		case NOT_COMPLETE:
			label_text_color = RED;
			label_text_id = IDC_STATIC_PRODUCT_TEST_RESULT;
			resultstring = "PRODUCT TEST RESULT: Not Complete";
			GetDlgItem(IDC_STATIC_PRODUCT_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		case NO_AVAIL_LICENSE_KEY:
			label_text_color = RED;
			label_text_id = IDC_STATIC_PRODUCT_TEST_RESULT;
			resultstring = "PRODUCT TEST RESULT: FAILURE No License Key";
			GetDlgItem(IDC_STATIC_PRODUCT_TEST_RESULT)->SetWindowText(resultstring); 
		break;
		...
		default:
			label_text_color = RED;
			label_text_id = IDC_STATIC_PRODUCT_TEST_RESULT;
			resultstring = "PRODUCT TEST RESULT: FAILURE";
			GetDlgItem(IDC_STATIC_PRODUCT_TEST_RESULT)->SetWindowText(resultstring); 
	}
#ifdef update_color	
